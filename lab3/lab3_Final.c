...
int main(){	
	...
	while(1){
		...
		for(int i = 0; i<=3; i++){			//Loop through for 4 presses of the button
			...
			initLink(&newLink);				//Create a new list element
			
			newLink->e.itemCode =  (PINA & 0x03);	//Read the input bits and place them into the itemCode variable of the new element
			enqueue(&head, &tail, &newLink);		//Add the new element to the head of the list
			
		}//for - Input Loop
		dequeue(&head, &rtnLink);					//Discard the first element in the list
		free(rtnLink);								//Free the memory of the discarded element
		...
		for(int j = 0; j<3; j++){
			...
			free(rtnLink);						//Free the memory occupied by the extracted element

		}//for - Display loop
		
		while((PINA & 0x04) == 0x04);			//Wait for the Button to be pressed (High on Port A Pin3)
		timerCount(20);							//Debouncing delay of 20ms
		while((PINA & 0x04) == 0x00);			//Wait for the button to be pressed (Low on Port A Pin3)
		timerCount(20);							//Debounce delay of 20ms
